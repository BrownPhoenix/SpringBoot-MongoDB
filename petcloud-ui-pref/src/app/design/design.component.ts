  ...
  ngOnInit() {
    this.httpClient.get('http://localhost:8080/ingredients')
        .subscribe(data => {
          ...
          this.wraps = this.allIngredients.filter(w => w.type === 'ANIMAL');
          this.proteins = this.allIngredients.filter(p => p.type === 'GENDER');
          this.veggies = this.allIngredients.filter(v => v.type === 'COLOUR');
          this.cheeses = this.allIngredients.filter(c => c.type === 'UNIQUE_DETAIL');
          this.sauces = this.allIngredients.filter(s => s.type === 'PERSONALITY');
        });
  }
  // end::ngOnInit[]
